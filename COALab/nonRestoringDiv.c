...
int *addN(const int *a, const int *b, int size)
{
...
void nonRestoringDivisionAlgo(int dividend, int divisor)
{
  int n = 4;
  int AC[4] = {0};
  int Q[4];
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      int *addRes = addN(AC, M, n);
      memcpy(AC, addRes, sizeof(int) * n);
      free(addRes);
      printf("\t");
      printBits(AC, n);
      printf("\t");
...
int main()
{
  ...
  scanf("%d", &divisor);
  nonRestoringDivisionAlgo(dividend, divisor);
  return 0;
}
